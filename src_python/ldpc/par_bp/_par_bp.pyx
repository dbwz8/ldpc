...
cdef class ParBpDecoder(BpDecoderBase):
    ...
    def __cinit__(self, pcm: Union[np.ndarray, spmatrix], error_rate: Optional[float] = None,
                 ...
                 osd_order: int = 0, par_bp_iterations: int = 0, ppar_bp_frequency: int = 0, ppar_bp_seed: int = 0):
        
        self.MEMORY_ALLOCATED=False

        ## set up OSD with default values and channel probs from BP
        self.par_bp_iterations = par_bp_iterations
        self.parBp = new ParBpDecoderCpp(self.pcm[0], self.par_bp_iterations, ppar_bp_frequency, ppar_bp_seed)
    ...
    def decode(self, syndrome: np.ndarray) -> np.ndarray:
        ...
        if self.bpd.converge:
            ...
            par_bp_decoding = self.parBp.decode(bpd_syndrome)
            for i in range(self.n):
                out[i] = par_bp_decoding[i]
        return out
